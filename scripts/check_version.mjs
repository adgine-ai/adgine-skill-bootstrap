...
const REMOTE_VERSION_URL = "https://raw.githubusercontent.com/adgine-ai/adgine-skill-bootstrap/main/VERSION";
const RELEASE_URL = "https://github.com/adgine-ai/adgine-skill-bootstrap/releases/latest";
const CACHE_TTL_MS = 10 * 60 * 1000;
const REQUEST_TIMEOUT_MS = 5_000;
const CACHE_FILE = path.join(os.tmpdir(), "adgine-skill-bootstrap-version.json");
...
function writeCache(latest, now = Date.now()) {
  try {
    fs.writeFileSync(CACHE_FILE, `${JSON.stringify({ checked_at_ms: now, latest })}\n`, { mode: 0o600 });
  } catch {
...
async function fetchLatestVersion({ force = false } = {}) {
  ...
  try {
    ...
    if (!(testMode && process.env.ADGINE_BOOTSTRAP_VERSION_TEST_URL)) writeCache(latest);
    return latest;
  } finally {
    clearTimeout(timer);
  }
...
function formatUserInline(state) {
  ...
  if (state.install_type === "git") {
    return `Adgine Skill Bootstrap v${state.latest} 已发布（当前 v${state.current}）。请确认后更新到最新版本。`;
  }
  return `Adgine Skill Bootstrap v${state.latest} 已发布（当前 v${state.current}）。请前往 ${state.release_url} 下载最新版并在当前 Agent 中重新安装。`;
}
